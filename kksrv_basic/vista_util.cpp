...
const TCHAR* const kExplorer = TEXT("EXPLORER.EXE");
...
HRESULT FindProcessesInSession(
    DWORD session_id,
...
HRESULT GetPidsInSession(const TCHAR* exe_name, DWORD session_id, std::vector<DWORD>* pids) {
    pids->clear();

    HRESULT hr = FindProcessesInSession(session_id,
        exe_name,
        pids);
    if (FAILED(hr)) {
        return hr;
...
HRESULT GetProcessPidsForActiveUserOrSession(
...
HRESULT GetImpersonationToken(DWORD process_id, HANDLE* user_token) {
    ...
    if (!::OpenProcessToken(process.Get(), TOKEN_DUPLICATE | TOKEN_QUERY, &process_token)) {
